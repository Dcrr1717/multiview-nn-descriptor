# -*- coding: utf-8 -*-
# Regenera fig4_violins con el MISMO pipeline de datos que la version previa
# (fase10 records, KFold-5) pero con las anotaciones de significancia
# CALCULADAS (Wilcoxon unilateral pareado sobre los 5 folds), no decorativas.
import pickle
import numpy as np
import matplotlib
matplotlib.use("Agg")
...
# Wilcoxon unilateral pareado sobre los 5 folds (minimo alcanzable p=1/32)
...
vparts = ax.violinplot(auc_data, positions=positions, widths=0.5,
...
for i, aucs in enumerate(auc_data):
    ...
    ax.scatter(positions[i]+jitter, aucs, color=COLORS[i], s=18, zorder=3,
...
ax.set_xticklabels([d[0] for d in DESCR])
ax.set_ylabel("Family AUC (OvR, KFold-5)")
ax.set_ylim(0.65, 1.02)
ax.set_title("Descriptor family discrimination\n(violin = KFold-5 distribution)",
             fontsize=8)
fig.tight_layout()
for out in [os.path.join(ROOT, "figures", "fig4_violins")]:
    fig.savefig(out + ".pdf", format="pdf"); fig.savefig(out + ".png")
print("fig4_violins regenerada con estrellas calculadas:", stars(p1), stars(p2))
