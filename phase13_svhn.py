...
DATA_DIR = Path("./data")
OUT = Path("./outputs/fase13_svhn"); OUT.mkdir(parents=True, exist_ok=True)
PROG = OUT / "progress.json"
NC = 10  # SVHN (dígitos)
T0 = time.time()
print("="*70); print(f"  FASE 13: SVHN (n=40)  | device={DEVICE}"); print("="*70)

# ── Datos: PRECARGA EN VRAM (elimina el cuello de botella del DataLoader,
#    que con num_workers=0 dejaba la GPU al ~35%). Sin aumento de datos:
#    el dataset entero vive en la GPU y se entrena con lotes directos.
MEAN, STD = (0.4377, 0.4438, 0.4728), (0.1980, 0.2010, 0.1970)
BS = 256  # lotes grandes posibles porque no hay E/S por lote
def load_gpu(split):
    ds = datasets.SVHN(DATA_DIR, split=split, download=True)
    X = torch.tensor(ds.data, dtype=torch.float32).div_(255.0)   # (N,3,32,32)
...
Xtr, ytr = load_gpu("train"); Xte, yte = load_gpu("test")
gb = (Xtr.element_size()*Xtr.nelement() + Xte.element_size()*Xte.nelement())/1e9
print(f"  SVHN en VRAM: train {tuple(Xtr.shape)} | test {tuple(Xte.shape)}  ({gb:.2f} GB)", flush=True)
...
RECPATH = OUT/"all_records_f13.pkl"
...
for idx,(kind,cfg) in enumerate(ALL):
    ...
    try:
        ...
        acc, acts = evaluate_extract(model, is_mlp, extract_n=1280)
        ...
        print(f"  [{idx+1:02d}/40] {cfg['fam']:11s} {kind} acc={acc:5.2f}%  ({dt}s)  acts={A.shape}", flush=True)
...
print(f"\n  SVHN acc=[{accs.min():.2f}, {accs.max():.2f}]  rango={accs.max()-accs.min():.2f} pp")
print(f"  Registros: {OUT/'all_records_f13.pkl'}  |  Tiempo total: {round(time.time()-T0)}s")
print("="*70); print("  FASE 13 COMPLETADA"); print("="*70)
